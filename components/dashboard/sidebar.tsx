...
interface SidebarProps {
  activeView: string
  onViewChange: (view: 'home' | 'record' | 'library' | 'insights' | 'settings') => void
}
...
export function Sidebar({ activeView, onViewChange }: SidebarProps) {
  ...
  const navItems = [
    ...
    { id: 'record', label: 'Record', icon: MicIcon, description: 'Start new session' },
    { id: 'library', label: 'Library', icon: LibraryIcon, description: 'Browse recordings' },
    { id: 'insights', label: 'Insights', icon: ChartIcon, description: 'View patterns' },
  ...
  return (
    <aside className="w-72 border-r border-border bg-gradient-to-b from-card via-card to-muted/20 flex flex-col relative overflow-hidden">
      ...
      <div className="absolute bottom-20 right-10 w-32 h-32 bg-accent/5 rounded-full blur-3xl animate-float" style={{ animationDelay: '1s' }} />
      ...
      <div className="p-6 border-b border-border/50 backdrop-blur-sm relative z-10">
        <div className="flex items-center gap-3 group cursor-pointer">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground tracking-tight">CultureLens</h1>
            <p className="text-xs text-muted-foreground leading-tight">Understanding Through Listening</p>
          </div>
        </div>
      </div>
