...
async function setup() {
  try {
    ...
    if (platform === "win32") {
      ...
      await runCommand(venvPip, ["install", "-r", "requirements.txt"], {
        cwd: backendDir,
      });
      await runCommand(venvPip, ["install", "-r", "requirements-dev.txt"], {
        cwd: backendDir,
      });
    } else {
    ...
    if (!fs.existsSync(rootEnv)) {
      console.log(
        "⚠️  .env file not found in root. Please copy .env.example to .env and fill in your API keys."
      );
    } else {
    ...
    if (!fs.existsSync(backendEnv)) {
      console.log(
        "⚠️  .env file not found in backend/. Please copy backend/.env.example to backend/.env and fill in your API keys."
      );
    } else {
