...
async function testGeminiAnalysis() {
  ...
  try {
    ...
    const sessionRes = await fetch(`${BASE_URL}/api/sessions`, {
      ...
      headers,
      body: JSON.stringify({
        consent: { personA: true, personB: true, timestamp: new Date().toISOString() },
        settings: {
    ...
    if (!transcriptRes.ok) {
      throw new Error(`Transcript save failed: ${JSON.stringify(transcriptData)}`);
    }
    ...
    const analyzeRes = await fetch(`${BASE_URL}/api/sessions/${sessionId}/analyze`, {
      method: "POST",
      headers
    });
    ...
    if (result.metrics) {
      console.log("\n\n📊 METRICS:");
      console.log("-".repeat(40));
      console.log(`Talk time: A=${result.metrics.talkTimeMs?.A}ms, B=${result.metrics.talkTimeMs?.B}ms`);
      console.log(`Turn count: A=${result.metrics.turnCount?.A}, B=${result.metrics.turnCount?.B}`);
      console.log(`Interruptions: A=${result.metrics.interruptionCount?.A}, B=${result.metrics.interruptionCount?.B}`);
    }
...
async function testGeminiDirectly() {
  ...
  try {
    ...
    const { analyzeTranscriptWithGemini } = await import("../lib/gemini-analysis.ts");
    
    const segments = [
      { speaker: "A", text: "I think we need to talk about the project timeline.", startTime: 0, endTime: 5000 },
      { speaker: "B", text: "Can you be more specific about which milestones you're worried about?", startTime: 5000, endTime: 10000 },
      { speaker: "A", text: "The design phase was supposed to be done last week.", startTime: 10000, endTime: 15000 },
      { speaker: "B", text: "You're right. The extra time was necessary for stakeholder buy-in.", startTime: 15000, endTime: 20000 },
    ];
